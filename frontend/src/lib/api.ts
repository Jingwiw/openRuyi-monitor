...
export async function api<T>(path: string): Promise<{status: number; data: T | null}> {
  try {
    const response = await fetch(backendURL(path), {signal: AbortSignal.timeout(8000), redirect: 'error'});
